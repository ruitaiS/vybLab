...
#clear out bottom half of image (set rest to 0.01)
digits_train_imgs_u = digits_train_imgs.copy()
digits_test_imgs_u = digits_test_imgs.copy()
def splicer(switch):
    if(switch == "u"):
        for i in range(0, len(digits_train_imgs_u)):
            for j in range(392, 784):
                digits_train_imgs_u[i][j] = 0.01
        for i in range(0, len(digits_test_imgs_u)):
            for j in range(392, 784):
                digits_test_imgs_u[i][j] = 0.01
    if(switch == "l"):
        for i in range(0, len(digits_train_imgs_u)):
...
def iterate():
    ...
    #Train Digit NN
def train(NN):
    for i in range(len(digits_train_imgs_u)):
        Digit_NN.train(digits_train_imgs_u[i], digits_train_labels_one_hot[i]) #    Digit_NN.train(digits_train_imgs[i], digits_train_labels_one_hot[i])
