...
class NeuralNetwork:
    ...
    def __init__(self,
        ...
        #self.create_weight_matrices()  #first we run v1 with  this to initialize the weights of the hidden layer, then we run load to preserve the network
        self.load_weight_matrices()

    def load_weight_matrices(self):
        with open("wih_init.txt", "rb") as fp:   # Unpickling
            self.wih = pickle.load(fp)
        with open("who_init.txt", "rb") as fp:   # Unpickling
            self.who = pickle.load(fp)
    ...
    def train(self, input_vector, target_vector):
        ...
        NeuralNetwork.output_hidden = activation_function(output_vector1)

        output_vector2 = np.dot(self.who, NeuralNetwork.output_hidden)
        output_network = activation_function(output_vector2)

        output_errors = target_vector - output_network
        # update the weights:
        tmp = output_errors * output_network * (1.0 - output_network)
        tmp = self.learning_rate  * np.dot(tmp, NeuralNetwork.output_hidden.T)
        self.who += tmp
        ...
        # update the weights:
        tmp = hidden_errors * NeuralNetwork.output_hidden * (1.0 - NeuralNetwork.output_hidden)
